	...
	<div class="ct-container container site-content wpwa-container">
		...
		<?php while ( have_posts() ) : the_post(); 
            ...
            if ( class_exists( '\\Elementor\\Plugin' ) ) {
                $is_elementor = \Elementor\Plugin::$instance->db->is_built_with_elementor( $product_id );
            }

            if ( $is_elementor ) {
                // Let Elementor handle the whole layout
                the_content();
            } else {
                // Fallback to hardcoded template for non-Elementor products
                            ...
                            <div class="wpwa-tab-panel <?php echo ! $has_custom ? 'active' : ''; ?>" id="wpwa-tab-reviews">
                                ...
                                <div class="wpwa-review-form-wrapper">
                                    ...
                                    comment_form( array(
                                        ...
                                        'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( 'Review', 'noun', 'webesia-wa-product-catalog' ) . '</label><textarea id="comment" name="comment" cols="45" rows="5" aria-required="true" class="widefat" placeholder="' . esc_attr__( 'Write your experience using this product...', 'webesia-wa-product-catalog' ) . '"></textarea></p>',
                                        'fields'               => array(
                                            'author' => '<div class="wpwa-form-row' . ( $req ? ' required' : '' ) . '"><label for="author">' . esc_html__( 'Name', 'webesia-wa-product-catalog' ) . '</label> <input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' class="widefat" placeholder="' . esc_attr__( 'Your Name', 'webesia-wa-product-catalog' ) . '" /></div>',
                                            'email'  => '<div class="wpwa-form-row' . ( $req ? ' required' : '' ) . '"><label for="email">' . esc_html__( 'Email', 'webesia-wa-product-catalog' ) . '</label> <input id="email" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' class="widefat" placeholder="' . esc_attr__( 'Email Address', 'webesia-wa-product-catalog' ) . '" /></div>',
                                        ),
                                    ) );
                                    ?>
                                </div>
